...
A ticker is worth the dollar it wraps, but a name with no market is a name no chart can price and no router can reach. So every invented ticker runs its own Uniswap v4 pool against USDG from the moment it is invented, at a fixed 0.05% fee, behind one hook shared by every ticker, `ManagedTickerHook`. The pool is an ordinary pool with ordinary swaps, no custom accounting and no dynamic fee, so any router that trades Uniswap v4 trades it; the hook only hands each pool's callbacks to the wrapper that owns it.

What the wrapper does with them. Its unsold inventory is the buy side: tokens it mints to itself, owed to nobody and backed by nothing until somebody buys them, when the dollars paid become their backing. At rest that offer is `INVENTORY_FLOOR`, a million dollars' worth, and it grows by four times whatever is in circulation, so one buy through the pool can take that much and fills whole or not at all. The wrapper's backing is the sell side: every dollar anyone paid in, on offer at a dollar each, so whatever is in circulation can always come back. Before every outside swap the wrapper rebuilds both positions and re-centres the price at exactly one dollar, paying for the tiny re-centring swap from its own surplus, never from the backing; after the swap it checks that the fill was whole, that the price is within two ticks of a dollar, and that the backing still covers the circulation. If any of that fails the swap reverts and nothing moved. One visit to the pool per transaction; a route that crosses the same name's pool twice reverts.

The surplus that pays for re-centring is the ticker fee, converted to USDG in the launch transaction and handed to the wrapper as a donation that nobody can redeem, plus the pool's own fees as they accrue. Backing covers circulation before and after every operation, and `mint` and `redeem` still work one for one at any time, so the pool cannot be pushed off a dollar at any size: a buy above the offer or a sell above the circulation reverts whole. Only the wrapper adds or removes liquidity in its pool, and only while it is maintaining; the pool holds no position of anyone else's. Anyone can open a second WRAPPER/USDG pool without the hook, at any price, because Uniswap pools are permissionless; that pool is theirs, not the ticker's, and every trade in it away from a dollar hands an arbitrage to whoever mints or redeems the wrapper.

## Activation: the two buys after a launch

Chart sites and trackers price a name from a swap that lands in a wallet after the name's pool exists, and price a coin from a buy after its pool exists. The launch transaction is neither: the pools are created in it, and a first buy inside it is a buy inside the transaction that created the pool. So a launch is not finished until two more transactions land, one after the other: the name itself, bought through its own pool and paid straight to the creator's wallet by the pool manager (`ZapRouter.zapTicker`), then the coin (`ZapRouter.zapBuy`). Under a name that has already been through its first buy, the coin's buy alone is enough. The site makes this the last step of creating a coin, about twenty dollars each, and shows a coin as not yet activated until both have landed; the coin trades normally on tickr either way.
