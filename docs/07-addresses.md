...
| `BuybackTreasuryHoly` | The fee recipient for launches made from 13 September 2026 on, and HOLY's creator-fee recipient: the same treasury with HOLY as the coin it buys and burns, plus `burnCoin()` for the HOLY it receives directly. No owner | `0x29d6427dc78b9B405DFAA118A9D634cE4CEf73f4` |
| `TickerLauncher` | Invented redeemable names (one-for-one USDG wrappers, each running its own dollar pool) and launched coins under them; the ticker club; no owner. Unregistered from the factory on 13 September 2026, so it launches nothing new; the names and coins it made keep trading | `0x7f6c8bA781b5bDC499F2BA7501A2178508877649` |
