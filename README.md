...
a memecoin launchpad on Robinhood Chain (chain id 4663). one transaction puts a coin's whole supply into a Uniswap v4 pool that is locked from the first block, priced in ETH, USDG, a Robinhood Stock Token, a coin launched here, or a name the creator invents. the pool's fee is the only fee: the quote side is split between the creator and the protocol, the coin side burns. the protocol's share buys and burns the official coin. a buy in a coin's first five seconds pays a snipe tax that starts at 99% and is gone by the fifth second.
