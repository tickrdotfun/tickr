...
In the genesis transaction, while launching was still closed to everyone but the deployer: `TickerLauncher.launchAndBuy("FUN", ...)` invented FUN, paid the ticker fee that became the working dollars of FUN's own pool, opened TICKR's pool with the whole supply locked in it, on the same terms as every coin after it, and made the disclosed first buy, all in that one transaction, so nothing could trade the pool before it. Two small buys followed, each its own transaction, both to the deployer's wallet: FUN through its own pool, then TICKR; these are the activation buys every coin needs before chart sites price it, see below. Then launches opened to everyone.
