...
export function LaunchGuardLine({ d }: { d: TokenData }) {
  ...
  const fresh = now > 0 && !!launch && now - Number(launch.launchedAt) < 120;
  ...
  return <p className="detail-note detail-note-tight">launch protection is on: the first two blocks, 5% of supply per wallet. sells are never limited.</p>;
}
