 ...
 * that volume gives it in the club's pots. Nobody owns the ticker; this is the only thing a coin under it earns
 * from the others.
 */
            ...
            <tbody>
              {c.members.map((m) => (
                <tr key={m.token} data-me={sameAddr(m.token, launch.token)}>
                  ...
                  <td className="num">{m.lastVolume > 0n ? pct(m.lastWeight, 1) : "0%"}</td>
