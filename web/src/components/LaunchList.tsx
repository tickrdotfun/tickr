...
/**
 * v2 is an era, not a fee: everything launched from the first coin on the v2 terms onward, whatever it is priced
 * in and whatever creator tax it carries (a tax raises the pool fee, so the fee alone called a taxed v2 launch "v1"
 * and hid it under the deprecated tab). The first v2 launch is the fifteenth in the factory's list.
 */
const V2_FIRST_LAUNCH = 15;
const V2_POOL_FEE = MARKET_BASE_FEE_BPS * 100;
const versionOf = (r: Row): VersionKey => (r.launch.index >= V2_FIRST_LAUNCH || r.launch.poolFee === V2_POOL_FEE ? "v2" : "v1");
    ...
    <Link href={`/t/${r.launch.token}`} className="coin-card st-signal no-underline hover:no-underline">
      ...
      <div className="coin-body">
        <div className="flex items-baseline gap-2 min-w-0">
          ...
          <CopyIcon address={r.launch.token} />
        </div>
