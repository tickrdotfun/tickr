...
contract LaunchLocker is IERC721Receiver, ReentrancyGuard {
    ...
    {
        uint256 base = (coinOut * p.hookFeeBps) / (uint256(p.hookFeeBps) + creatorTaxBps);
        burnedCoin = (base * p.protocolShareBps) / BPS + (base * p.clubShareBps) / BPS;
        creatorCoin = coinOut - burnedCoin;
        if (burnedCoin > 0) IERC20(token).safeTransfer(BURN, burnedCoin);
        if (creatorCoin > 0) {
            IFeeEscrow escrow = IFeeEscrow(IFactory(factory).feeEscrow());
            IERC20(token).forceApprove(address(escrow), creatorCoin);
            escrow.creditToken(IFactory(factory).creatorFeeRecipientOf(token), token, creatorCoin);
        }
    }
