...
contract MarketOnboardingForkTest is Test {
    ...
    function setUp() public {
        ...
        tp.expectedEconomics = launcher.previewEconomics(configId, TESTNAME);
    ...
    function test_fork_aNameOnboardsItselfAndTheProceedsAreAllocated() public {
        if (!forked) return;
        assertEq(uint256(registry.kindOf(TESTNAME)), 0, "the registry has never been told about this name");
        assertEq(
            uint256(registry.provenanceOf(TESTNAME)),
            uint256(2),
            "but its issuer claims it, which is the only authority that matters"
        );
        assertEq(vault.minRateToCounterX96(TESTNAME), 0, "and no floor was set for it by hand");
        assertEq(
        ...
        assertEq(uint256(registry.kindOf(TESTNAME)), 2, "the collect recorded it on the way past");
        assertEq(IERC20(TESTNAME).balanceOf(team), 0, "and no name was forwarded whole to the team");
    }
    ...
    function test_fork_theDefaultFloorStillRefusesWeakTerms() public {
        ...
        uint256 held = IERC20(TESTNAME).balanceOf(address(vault));
        assertEq(held, owed, "the name was claimed but not one unit of it was sold on refused terms");
        assertEq(IERC20(TESTNAME).balanceOf(team), 0, "and it was not given away either");
        ...
        assertLt(IERC20(TESTNAME).balanceOf(address(vault)), held, "so the refusal was the terms, not the name");
    }

    /// @notice The owner may raise the default but never lower it below the frozen policy.
    function test_fork_theDefaultFloorCannotBeWeakened() public {
        if (!forked) return;
        vm.prank(owner);
        vm.expectRevert();
        vault.setDefaultMinRate(true, FeeSettings.MIN_RATE_NAME_TO_COUNTER_X96 - 1);

        vm.prank(owner);
        vault.setDefaultMinRate(true, FeeSettings.MIN_RATE_NAME_TO_COUNTER_X96 + 1);
        assertEq(vault.defaultMinRateToCounterX96(), FeeSettings.MIN_RATE_NAME_TO_COUNTER_X96 + 1);
    }
    ...
    function test_fork_aNewLegacyWrapperCoinPaysV2AndRedeemsExactly() public {
        ...
        console.log("legacy wrapper: owed | collected | team | earmarked");
        console.log(owed);
        console.log(total);
        console.log(toTeam);
        console.log(earmarked);
        ...
        assertGe(
            IERC20(USDG).balanceOf(address(vault)) + toTeam - usdgBefore,
            owed,
            "one wrapper unit became one dollar: a redemption, not a swap"
        );
        assertGt(earmarked, 0, "and the buyback got its share of it");
